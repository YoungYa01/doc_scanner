...
from doc_scanner.DocScanner import DocScanner
...
def initialize_scanner(is_tf):
    global scanner
    if scanner is None:
        if is_tf:
            model_path = getattr(settings, 'DOCUMENT_SCANNER_TF_MODEL_PATH', None)
            if model_path:
                logger.info(f"正在加载模型: {model_path}")
            scanner = DocScanner(model_path)
        else:
...
def document_scan(request):
    ...
    if scanner is None:
        initialize_scanner(is_tf=False)
